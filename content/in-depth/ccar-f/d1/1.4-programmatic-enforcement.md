...
A prompt instruction is a **request to a probabilistic system** — one that weighs likelihoods
rather than following fixed rules, so the same instruction is obeyed nearly always rather than
always. A code check is a **property of the system**: it either holds or the program is broken. Those aren't two points on a reliability scale — they're different kinds of
