...
A **multi-agent system** is what you get when one agent isn't enough, so you run several — each on
a slice of the work, with one of them in charge. The one in charge is the **coordinator**; the
others are **subagents**, or workers.

Here is the situation that forces it. A sales team asks: "compare how Acme, Globex and Initech
price their product." One agentic loop can do this. It fetches page one, reads it, fetches page
two, reads it, fetches page three, reads it, then writes the comparison — with all three pages
sitting in a single **context window**, the working memory one request can hold, each page resent
on every later turn.

Three problems show up immediately. It's serial, so **latency** — the time from asking to getting
an answer — is the sum of all three fetches rather than the slowest one. It's expensive, because
page one is paid for again on every turn after it was read. And quality degrades: by the time the
model writes the comparison, the useful facts are buried under tens of thousands of **tokens**
(roughly word-pieces, the unit the model reads and bills by) of raw HTML it no longer needs.
