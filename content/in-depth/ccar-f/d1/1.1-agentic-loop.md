...
A plain chatbot answers from what it already knows. Ask it why a test is failing and it can only
guess, because it has never seen the test.

An **agent** is what you get when you let the model do something *first* and answer afterwards —
read the file, run the suite, look at the real error. The **agentic loop** is the machinery that
makes that possible: the model asks for an action, your code performs it, the result goes back to
the model, and the cycle repeats until the model says it is done. "Loop" is literal — the same
few steps run again and again.

Here is why it cannot be a single round trip. Your CI pipeline goes red at 02:00 and a test in
`payment_test.py` fails. Someone has to read the test, run it, see the actual error, change a
line, and re-run to confirm. That is four or five separate acts of looking-then-deciding, and
whether it turns out to be five rather than two or twenty depends on what the error is. You cannot
know that number when you send the first request — so you cannot write it as a fixed sequence of
calls. The loop is how the count stays open.

Every other pattern in this domain — subagents, hooks, decomposition, session forking — is this
loop with something bolted on. Learn it once here and the rest are variations on it.

The whole design rests on one decision your code makes on every pass: **call the model again, or
finish?** Take that decision from the wrong signal and the agent stops before the fix is verified,
or spins until you run out of money. The API gives you the right signal explicitly, and the most
common production bug in this domain is not using it.

> **🗣️ In plain English:** A chatbot answers from memory. An agent is allowed to go and check first — and the loop is what lets it check more than once.

## Meet the Moving Parts

Three participants. Confusing their jobs is where most beginners go wrong.
...
- **Tools** are the menu Claude orders from. Each is a name, a description, and a **schema**: the
  declared shape of the input, saying which fields exist, of what type, and which are required.
  Claude picks from the menu; your code cooks.

Two consequences that matter later:

**Claude never executes anything.** It emits a `tool_use` block — a *request*, not an action. There
is always a layer of your own code between "Claude asked" and "it happened". That gap is exactly
where hooks (1.5) and programmatic enforcement (1.4) live.

**The API is stateless** — it remembers nothing between calls. Every request carries the whole
conversation with it. What looks like a continuing chat is really the `messages` list you resend
each time, growing by two entries per turn: Claude's response, then your tool results. The loop is
you rebuilding and resending that list.

And the signal that ends it: every response carries a **`stop_reason`** field saying why Claude
stopped. `tool_use` means "I need something done — call me back with the result". `end_turn` means
"I am finished". Your code branches on that field and nothing else.
...
> **🗣️ In plain English:** Claude is the brain, your code is the hands, and `stop_reason` is how the brain says either "your turn" or "we're done".
...
sequenceDiagram
    ...
    CI->>AL: test failed - payment_test.py
    AL->>C: task plus 4 tools
    C-->>AL: tool_use - read the test
    AL->>C: tool_result - file text
    C-->>AL: tool_use - run the suite
    AL->>C: tool_result - assertion output
    C-->>AL: tool_use - edit then re-run
    AL->>C: tool_result - tests pass
    C-->>AL: end_turn plus summary
    AL->>CI: open PR
```
