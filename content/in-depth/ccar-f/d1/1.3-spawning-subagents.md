...
**fan out** — split one request into many running side by side — but never does is very often just
missing that one entry.
...
- **Sources.** URLs, file paths, page numbers. This is **provenance** — the record of where a fact
  came from — and provenance that isn't carried forward becomes an unsourced claim by the time it
