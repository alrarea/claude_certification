...
- **State is checkpointed** — saved as it goes, so the work can resume from the last save rather
  than the beginning. A crash forty calls in resumes from `visited` rather than paying for
