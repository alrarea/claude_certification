...
Nothing malfunctioned. The **guardrail** — a check that sits outside the model and stops it doing
something it shouldn't — simply wasn't there, because it lived on one laptop.
...
5. **Nothing enforces any of it.** CLAUDE.md is instruction, not enforcement — the 1.4 distinction.
   A rule that must hold every time needs a **hook** (1.5) — code your system runs automatically
   in front of a matching tool call, which can refuse it — or a permission deny rule.
