...
A **hook** in that last row is code your system runs automatically in front of a matching tool
call, which can refuse it — a command is something a person chooses to invoke, so it can never
be the answer when the rule has to hold every time.
