...
might be 24 hours. If no, you are very likely paying double for **latency** — the time from asking to getting an
answer — that nobody is consuming.
...
3. **Put the shared prefix — the identical opening every request begins with — in a cached system
   prompt.** Batch and caching compose; the rubric is
