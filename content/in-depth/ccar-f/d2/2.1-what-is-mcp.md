...
**MCP — the Model Context Protocol** — is a standard way to connect an AI assistant to an outside
system. You wrap the system once in a **server** that describes what it can do; any MCP-compatible
**client** — Claude Code, a Slack bot, a web console you haven't built yet — connects to that
server and discovers those capabilities, with no bespoke integration code at either end.

Here is the problem that standard exists to solve.
...
Without a standard, that's three integrations. Each one re-declares the same tool **schemas** — the
declared shape of a tool's input, which fields exist and which are required — re-implements the
same authentication, and invents its own way of signalling that the lookup failed. When the
...
MCP fixes this by standardising the *shape* of the connection rather than the connection itself.
Wrap the rotation service once as an MCP server and every client reaches it the same way. The
USB-C comparison is the right one, and it's worth being precise about what it buys you: not fewer
systems, but one connector shape, so the number of integrations stops multiplying.
...
- **The transport** is how the two actually talk: **stdio** — the client launches the server as a
  local process and speaks over its standard input and output — for a local server, or HTTP for a
  remote one. Same protocol either way.
