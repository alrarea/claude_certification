...
A **guardrail** is a check that sits outside the model and stops an unsafe action regardless of
what the model decided. This topic is about where those checks belong, and what happens when the
only one you have is the prompt itself.
...
hook** inspecting the proposed `terminate_cluster` call would have blocked it
**deterministically** — the same input producing the same refusal every time — regardless of how
persuasive the log line was.
