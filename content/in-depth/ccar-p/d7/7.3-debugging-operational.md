...
invalidated the cache prefix — the identical opening every request shares, which is what caching
matches on — and every request reverted to full-price uncached tokens.
...
the cache breakpoint, the marker saying everything before it can be reused. Structurally: cache hit rate became a monitored metric with an alert at 20
