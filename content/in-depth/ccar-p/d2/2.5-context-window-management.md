...
The **context window** is the total amount of text one request can hold — everything you send
plus everything the model writes back. Managing it is deciding what earns a place inside it.
...
- **The instruction module is cached, the case content isn't.** The module is stable across cases;
  case data varies per request and belongs after the breakpoint — the marker saying "everything
  before here is cacheable" (2.4).
